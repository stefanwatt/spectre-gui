...
func ReplaceLine(path string, row int, replacement string) error {
	...
	err := cmd.Run()
	if err != nil {
		log.Debug("Error replacing line with sed")
		log.Error(err.Error())
		return err
	}
	return nil
...
func GetReplacementText(
	...
	if err := cmd_sed.Start(); err != nil {
		log.Debug("[GetReplacementText] Error starting sed command:")
		log.Error(err.Error())
		return "", err
	}

	if err := cmd_echo.Run(); err != nil {
		log.Debug("[GetReplacementText] Error running echo command:")
		log.Error(err.Error())
		return "", err
	}

	if err := cmd_sed.Wait(); err != nil {
		log.Debug("[GetReplacementText] Error waiting for sed command:")
		log.Error(err.Error())
		return "", err
	}
