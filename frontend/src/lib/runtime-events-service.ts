...
import { windowContentRowMap, layout, cursor, pickers, cmdline, nestedState, getFloatingWindows, setFloatingWindows, completion, setCompletionDocumentation, fileExplorer } from "$lib/state.svelte"
import {
...
export function startListening() {
  ...
  // When neovim UI is attached, send actual window dimensions so the grid is sized correctly
  Events.On('neovim-ready', () => {
    App.OnResize(window.innerWidth, window.innerHeight);
  });
  ...
  Events.On('file-explorer-open', (ev) => {
    const data = ev.data;
    fileExplorer.open = true;
    fileExplorer.currentDir = data.currentDir;
    fileExplorer.parentEntries = data.parentEntries;
    fileExplorer.centerWindowId = data.centerWindowId;
    fileExplorer.previewWindowId = data.previewWindowId;
  });

  Events.On('file-explorer-close', () => {
    fileExplorer.open = false;
  });

  Events.On('file-explorer-preview', (ev) => {
    const data = ev.data;
    fileExplorer.previewIsFile = data.isFile;
    if (!data.isFile) {
      fileExplorer.previewEntries = data.entries;
    }
  });
}
